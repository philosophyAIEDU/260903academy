 ...
 * - SIGUN_NM만 원본 API 파라미터로 전달해 데이터량을 줄입니다.
 * - EMD_NM(읍면동), 업종구분(indutypeDivNm), 학원명(faclyNm)은 원본 API에 정확일치로
 *   넘기면(특히 업종구분은 실제 저장된 표기를 확신할 수 없어) 값이 조금만 달라도
 *   결과가 0건이 되는 문제가 있어, SIGUN_NM으로 받아온 결과에 대해 여기서
 *   부분일치(includes) 필터링으로 안전하게 처리합니다.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    const rawRows = await fetchAllAcademies({ sigunNm });
    ...
    if (indutypeDivNm) {
      const needle = indutypeDivNm.toLowerCase();
      items = items.filter((a) => a.industryType.toLowerCase().includes(needle));
    }
    if (faclyNm) {
