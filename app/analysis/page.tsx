...
export default function AnalysisPage() {
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-30 overflow-hidden bg-gradient-to-r from-emerald-600 via-teal-600 to-sky-600 shadow-lg shadow-emerald-900/10">
        <div
          className="pointer-events-none absolute -left-14 -top-16 h-52 w-52 animate-blob rounded-full bg-white/10 blur-3xl"
          aria-hidden="true"
        />
        <div
          className="pointer-events-none absolute -right-10 -top-24 h-64 w-64 animate-blob rounded-full bg-sky-200/25 blur-3xl [animation-delay:2s]"
          aria-hidden="true"
        />
        <div className="relative mx-auto flex max-w-7xl items-center gap-3 px-4 py-4 sm:px-6">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-xs leading-tight text-emerald-50">
      ...
      <main className="mx-auto flex max-w-7xl flex-col gap-4 p-4 sm:gap-5 sm:p-6">
        ...
        {options && options.sido.length === 0 && !optionsError && (
          <div className="flex items-start gap-2.5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3.5 text-sm text-amber-700">
            <AlertIcon className="mt-0.5 h-4 w-4 shrink-0" />
        ...
        {loading && (
          <div className="flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white p-10 text-sm text-slate-400 shadow-panel">
        ...
        {!loading && !hasAnalyzed && options && options.sido.length > 0 && (
          <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-panel">
            <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-50 to-sky-50 text-emerald-500 ring-1 ring-emerald-100">
        ...
        <footer className="pb-2 pt-1">
          <DataSourceNotice
            ...
            referenceLabel={
              options?.meta.dataReferenceMonth ? `${options.meta.dataReferenceMonth} (일부 지역만 반영)` : "확인 중"
            }
            message="실시간 정보가 아니며, 아직 일부 지역 데이터만 반영되어 있습니다. 실제 운영 여부는 현장 확인이 필요합니다."
          />
        </footer>
      </main>
    </div>
