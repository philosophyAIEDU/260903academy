...
export default function AcademyPage() {
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-30 overflow-hidden bg-gradient-to-r from-indigo-700 via-indigo-700 to-violet-800 shadow-lg shadow-black/40">
        <div
        ...
        <div className="relative mx-auto flex max-w-7xl items-center gap-3 px-4 py-4 sm:px-6">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-xs leading-tight text-indigo-100/80">
          ...
          <span className="hidden shrink-0 items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 text-xs font-medium text-white ring-1 ring-white/15 backdrop-blur lg:flex">
      ...
      <main className="mx-auto flex max-w-7xl flex-col gap-4 p-4 sm:gap-5 sm:p-6">
        ...
        {!error && hasSearched && !loading && (
          <div className="flex animate-fade-up flex-col gap-3">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              {favoriteIds.size > 0 && (
                <label className="flex shrink-0 items-center gap-1.5 self-start rounded-lg border border-slate-800 bg-slate-900 px-3 py-1.5 text-xs font-medium text-slate-300 shadow-sm sm:self-auto">
                  <input
                    type="checkbox"
        ...
        {/* 모바일 전용 지도/목록 전환 탭 */}
        <div className="flex gap-1 rounded-lg bg-slate-900 p-1 lg:hidden">
          <button
            type="button"
          ...
          >
            ...
            {visibleAcademies.length > 0 && (
              <span className="rounded-full bg-indigo-500/15 px-1.5 py-0.5 text-[10px] font-semibold text-indigo-300">
        ...
        <div className="grid min-h-[560px] flex-1 grid-cols-1 gap-4 sm:gap-5 lg:grid-cols-5">
          <section
            className={`flex-col overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 shadow-panel lg:col-span-3 lg:flex ${
              mobilePanel === "map" ? "flex" : "hidden"
            }`}
          >
            <div className="flex items-center justify-between border-b border-slate-800 bg-gradient-to-r from-indigo-500/[0.08] via-slate-900 to-violet-500/[0.05] px-4 py-3">
          ...
          <section
            className={`flex-col overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 shadow-panel lg:col-span-2 lg:flex ${
...
function EmptyState({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center">
      <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-indigo-500/10 text-indigo-400 ring-1 ring-indigo-500/20">
