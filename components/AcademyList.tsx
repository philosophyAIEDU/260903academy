...
export default function AcademyList({
  ...
  if (academies.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center">
        <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-500/10 to-rose-500/10 text-amber-400 ring-1 ring-amber-500/20">
          <AlertIcon className="h-6 w-6" />
        </span>
        <div>
          <p className="text-sm font-medium text-slate-300">검색 결과가 없습니다</p>
          <p className="mt-1 text-xs text-slate-500">다른 조건으로 다시 검색해보세요</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <ul className="thin-scrollbar h-full divide-y divide-slate-800 overflow-y-auto">
        ...
        return (
          <li key={academy.id}>
            <button
              ...
              className={`flex w-full items-start gap-3 border-l-[3px] py-3.5 pr-4 text-left transition-colors hover:bg-slate-800/60 ${
                active
                  ? "border-indigo-500 bg-gradient-to-r from-indigo-500/10 to-violet-500/5 pl-[13px]"
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 flex items-start gap-1 text-xs text-slate-400">
                  <MapPinIcon className="mt-0.5 h-3 w-3 shrink-0 text-slate-500" />
                  <span className="truncate">{academy.roadAddress || academy.lotAddress || "주소 정보 없음"}</span>
                </p>
                <p className="mt-0.5 flex items-center gap-1 text-xs text-slate-400">
                  <PhoneIcon className="h-3 w-3 shrink-0 text-slate-500" />
                  <span className="truncate">{academy.tel || "전화번호 정보 없음"}</span>
                </p>
                <p className="mt-0.5 flex items-center gap-1 text-xs text-slate-400">
                  <BookIcon className="h-3 w-3 shrink-0 text-slate-500" />
