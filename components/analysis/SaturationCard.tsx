  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-panel sm:p-6">
      ...
      <p className="text-sm text-slate-700">
        <span className="font-semibold">{industryName}</span> 업종은 {saturation.baselineLabel}
        {saturation.ratio !== null ? (
          <>
            {" "}
            대비 <span className="font-semibold">{saturation.ratio}배</span> 밀집도입니다.{" "}
            {meta.verdict}.
          </>
        ) : (
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
        <div className="rounded-lg bg-slate-50 px-3 py-2.5">
          <p className="font-medium text-slate-500">이 지역</p>
          <p className="mt-0.5 text-sm font-semibold text-slate-800">
            {saturation.localCount.toLocaleString()}개 · {saturation.localSharePct}%
          </p>
        </div>
        <div className="rounded-lg bg-slate-50 px-3 py-2.5">
          <p className="font-medium text-slate-500">{saturation.baselineLabel}</p>
          <p className="mt-0.5 text-sm font-semibold text-slate-800">
